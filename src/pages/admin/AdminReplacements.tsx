...
export function AdminReplacements() {
  ...
  const load = () => {
    setLoading(true);
    authedFetch('/replacement-requests').then((r) => r.json()).then((data) => {
      setRequests(data || []);
      setLoading(false);
    });
  };
  ...
  const resolve = async (id: string, status: 'approved' | 'rejected') => {
    await authedFetch(`/replacement-requests/${id}`, {
      method: 'PUT',
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-slate-900 mb-6">Replacement Requests</h1>

      {loading ? (
        <div className="text-slate-500 flex items-center gap-2"><Loader2 className="w-5 h-5 animate-spin" /> Loading...</div>
      ) : requests.length === 0 ? (
        <p className="text-slate-500">No replacement requests.</p>
      ) : (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl p-6 w-full max-w-md">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-slate-900">Review Replacement</h2>
